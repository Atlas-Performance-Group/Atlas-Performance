...
export default async function IpDetailPage({ params }: { params: Promise<{ ip: string }> }) {
  ...
  if (!version) {
    return <ErrorState message={`"${rawIp}" não é um endereço IPv4 ou IPv6 válido.`} />;
  }

  if (isPrivateOrReservedIp(ip)) {
    return <ErrorState message={`${ip} é um IP privado/reservado — não há geolocalização pública para consultar.`} />;
  }
...
function ErrorState({ message }: { message: string }) {
  return (
    <div className="atlas-card p-6 flex flex-col gap-3">
      ...
      <Link href="/painel" className="atlas-btn-secondary self-start" style={{ color: "var(--ink)" }}>
