...
export function HistoricoTable({ initialItems, initialTotal }: { initialItems: IpRecord[]; initialTotal: number }) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/rastreador/ips?${params.toString()}`, { cache: "no-store" });
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="atlas-card p-6">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          ...
          <Link href="/api/rastreador/ips/export" className="atlas-btn-secondary text-sm" style={{ color: "var(--ink)" }}>
            Exportar CSV
          </Link>
        </div>
              ...
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <tr key={item.ip} style={{ borderTop: "1px solid var(--line-soft)" }}>
                    ...
                    <td className="py-2 pr-4 whitespace-nowrap">
                      <Link href={`/admin/rastreador/ip/${encodeURIComponent(item.ip)}`} className="atlas-btn-ghost text-xs">
                        Ver detalhes
                      </Link>
                    </td>
                  </tr>
